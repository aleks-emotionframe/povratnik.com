...
export type Case = { name: string; age: number; place: string; line: string; shapes: string; first: string; situation: string };
...
export function caseText(c: Case, texts: Record<string, unknown>): Case {
  ...
  return { ...c, name: node.name ?? c.name, place: node.place ?? c.place, line: node.line ?? c.line, shapes: node.shapes ?? c.shapes, first: node.first ?? c.first };
}
